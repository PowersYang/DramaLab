...
- **Series**: 系列，指一组分集项目的上层容器。通常用于管理共享角色、共享场景、共享道具，以及系列级模型和提示词配置。
- **Project**: 项目，通常对应一个具体分集或一次独立创作任务。项目里会包含文本、角色、场景、道具、分镜、视频任务和导出结果。
- **Asset**: 素材资产，指角色、场景、道具等可复用的视觉对象。每个素材通常会有描述、当前选中图片、候选变体和锁定状态。
- **Storyboard**: 分镜，是把文本剧情拆成一帧一帧画面规划后的结构化结果。每一帧可关联角色、场景、道具、提示词、对白和候选图/视频。

可以简单理解为：`Series` 管一组 `Project`，`Project` 管自己的 `Asset` 和 `Storyboard`，最后再进入视频生成与成片导出。
...
# 编辑 backend/.env 文件，填入 DASHSCOPE_API_KEY
```
