...
class BillingTransaction(BaseModel):
    ...
    charge_id: Optional[str] = Field(None, description="关联计费单 ID")
    source_event: Optional[str] = Field(None, description="触发该流水的事件")
    external_ref: Optional[str] = Field(None, description="外部系统引用，例如供应商账单号")
...
class BillingCharge(BaseModel):
    """任务计费单，用于承载“预扣-确认-回补-调整”的状态机。"""

    id: str = Field(..., description="计费单唯一标识")
    billing_account_id: str = Field(..., description="所属账本 ID")
    organization_id: Optional[str] = Field(None, description="所属组织 ID")
    workspace_id: Optional[str] = Field(None, description="所属工作区 ID")
    job_id: str = Field(..., description="关联任务 job_id")
    task_type: str = Field(..., description="任务类型")
    status: str = Field("held", description="计费单状态")
    estimated_credits: int = Field(0, description="预扣豆数")
    final_credits: Optional[int] = Field(None, description="最终扣费豆数")
    reserved_credits: int = Field(0, description="当前预占豆数")
    settled_credits: Optional[int] = Field(None, description="当前已结算豆数")
    refunded_credits: int = Field(0, description="累计已退款豆数")
    adjusted_credits: int = Field(0, description="累计手工调整豆数")
    pricing_mode: str = Field("fixed", description="计价模式")
    pricing_snapshot_json: Dict[str, Any] = Field(default_factory=dict, description="命中的定价规则快照与计价入参")
    cost_snapshot_json: Dict[str, Any] = Field(default_factory=dict, description="供应商成本快照")
    usage_snapshot_json: Dict[str, Any] = Field(default_factory=dict, description="用于结算的 usage 快照")
    settlement_reason: Optional[str] = Field(None, description="最近一次结算或释放原因")
    settled_at: Optional[datetime] = Field(None, description="结算完成时间")
    reconciled_at: Optional[datetime] = Field(None, description="最近一次对账修复时间")
    last_reconcile_error: Optional[str] = Field(None, description="最近一次对账失败原因")
    version: int = Field(1, description="计费单版本号")
    hold_transaction_id: Optional[str] = Field(None, description="预扣流水 ID")
    settle_transaction_id: Optional[str] = Field(None, description="结算/回补流水 ID")
    idempotency_key: Optional[str] = Field(None, description="幂等键")
    created_by: Optional[str] = Field(None, description="创建人 ID")
    updated_by: Optional[str] = Field(None, description="最后修改人 ID")
    created_at: datetime = Field(default_factory=utc_now, description="创建时间")
...
class BillingPricingRule(BaseModel):
    ...
    reserve_credits: int = Field(0, description="入队阶段预占的豆数上限")
    minimum_credits: int = Field(0, description="最小结算豆数")
    pricing_config_json: Dict[str, Any] = Field(default_factory=dict, description="usage/hybrid 计费配置")
    usage_metric_key: Optional[str] = Field(None, description="主 usage 指标键，例如 seconds 或 input_tokens")
...
class PaymentOrder(BaseModel):
    """支付订单；承载人民币支付态，再由账务层完成算力豆入账。"""

    id: str = Field(..., description="支付订单唯一标识")
    billing_account_id: Optional[str] = Field(None, description="目标账本 ID")
    organization_id: Optional[str] = Field(None, description="所属组织 ID")
    workspace_id: Optional[str] = Field(None, description="所属工作区 ID")
    user_id: Optional[str] = Field(None, description="创建订单的用户 ID")
    channel: str = Field(..., description="支付渠道，例如 alipay 或 wechat")
    status: str = Field("pending", description="支付订单状态")
    amount_cents: int = Field(..., description="订单金额，单位分")
    ...
    provider_mode: str = Field("mock", description="当前 provider 模式")
    provider_order_id: Optional[str] = Field(None, description="渠道下单号")
    provider_trade_no: Optional[str] = Field(None, description="渠道交易号")
    provider_buyer_id: Optional[str] = Field(None, description="渠道买家标识")
    provider_response_json: Dict[str, Any] = Field(default_factory=dict, description="支付渠道原始响应快照")
    exchange_snapshot_json: Dict[str, Any] = Field(default_factory=dict, description="人民币兑换算力豆快照")
    bonus_rule_snapshot_json: Dict[str, Any] = Field(default_factory=dict, description="赠送规则快照")
    base_credits: int = Field(0, description="基础到账豆数")
    bonus_credits: int = Field(0, description="赠送豆数")
    total_credits: int = Field(0, description="总到账豆数")
    qr_payload: Optional[str] = Field(None, description="二维码承载 payload")
    qr_code_svg: Optional[str] = Field(None, description="可直接渲染的二维码 SVG")
    qr_expires_at: Optional[datetime] = Field(None, description="二维码过期时间")
    paid_at: Optional[datetime] = Field(None, description="支付成功时间")
    expired_at: Optional[datetime] = Field(None, description="订单过期时间")
    cancelled_at: Optional[datetime] = Field(None, description="订单取消时间")
    failure_reason: Optional[str] = Field(None, description="失败原因")
    client_token: Optional[str] = Field(None, description="客户端追踪 token")
    idempotency_key: Optional[str] = Field(None, description="幂等键")
    created_by: Optional[str] = Field(None, description="创建人 ID")
    updated_by: Optional[str] = Field(None, description="最后修改人 ID")
    created_at: datetime = Field(default_factory=utc_now, description="创建时间")
...
class PaymentEvent(BaseModel):
    ...
    id: str = Field(..., description="支付事件唯一标识")
    payment_order_id: str = Field(..., description="所属支付订单 ID")
    organization_id: Optional[str] = Field(None, description="所属组织 ID")
    workspace_id: Optional[str] = Field(None, description="所属工作区 ID")
    event_type: str = Field(..., description="事件类型")
    from_status: Optional[str] = Field(None, description="变更前状态")
    to_status: Optional[str] = Field(None, description="变更后状态")
    event_payload_json: Dict[str, Any] = Field(default_factory=dict, description="事件载荷")
    created_by: Optional[str] = Field(None, description="创建人 ID")
    updated_by: Optional[str] = Field(None, description="最后修改人 ID")
    created_at: datetime = Field(default_factory=utc_now, description="创建时间")
    updated_at: datetime = Field(default_factory=utc_now, description="更新时间")
...
class BillingReconcileRun(BaseModel):
    """一次 reconciler 扫描与修复执行记录。"""

    id: str = Field(..., description="对账运行记录 ID")
    status: str = Field("completed", description="运行状态")
    dry_run: bool = Field(False, description="是否为 dry-run")
    scan_scope_json: Dict[str, Any] = Field(default_factory=dict, description="扫描范围")
    examined_count: int = Field(0, description="检查的 charge 数量")
    repaired_count: int = Field(0, description="修复的 charge 数量")
    skipped_count: int = Field(0, description="跳过的 charge 数量")
    error_count: int = Field(0, description="修复失败数量")
