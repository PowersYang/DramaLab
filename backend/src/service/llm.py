...
from backend.src.schema.models import Script, Character, Scene, Prop, StoryboardFrame, GenerationStatus
...
# ── 润色阶段的默认系统提示词 ──────────────────────
# 这些是内置默认值；用户也可以通过 PromptConfig 在项目级覆盖。
# 占位符说明：{ASSETS}=素材上下文，{DRAFT}=草稿提示词，{SLOTS}=R2V 槽位上下文
...
class ScriptProcessor:
    ...
    def parse_novel(self, title: str, text: str) -> Script:
        ...
        try:
            ...
            # 像 API Key 未配置这类 ValueError 直接往上抛
            raise
    ...
    def _create_script_from_data(self, title: str, original_text: str, data: Dict[str, Any]) -> Script:
        ...
        characters = []
        name_to_char = {}  # 用于后续角色变体关联
        llm_id_to_uuid = {}  # 记录 LLM 返回 ID 到本地 UUID 的映射
        ...
        for char_data in data.get("characters", []):
            ...
            char = Character(
                ...
                clothing=char_data.get("clothing"),  # 新版提示词可能会合并到 description，这里先兼容保留
        ...
        for frame_data in data.get("frames", []):
            ...
            if isinstance(dialogue_data, dict):
                ...
                dialogue_text = dialogue_data  # 兼容旧版纯字符串格式
...
{text}"""
    ...
    def _construct_prompt(self, text: str) -> str:
        ...
        这里只抽角色、场景、道具；
        分镜帧由后续的 `analyze_to_storyboard` 单独生成。
        """
        return f"""
  ...
  "recommendations": [
    {
      ...
      "negative_prompt": "负向提示词"
        ...
        try:
            ...
            # 先清掉外层可能包着的 markdown 代码块
            content = _strip_markdown_json(content)

            # 内容异常偏长时先截断，避免模型陷入重复输出
            if len(content) > 5000:
                logger.warning(f"Response too long ({len(content)} chars), truncating...")
                content = content[:5000]
                # 尽量截在对象闭合处，降低后续 JSON 修复难度
...
"""
        ...
        try:
            ...
            # 首轮解析失败时，再带上 response_format 强约束重试一次
    ...
    def polish_storyboard_prompt(self, draft_prompt: str, assets: List[Dict[str, Any]], feedback: str = "", custom_system_prompt: str = "") -> Dict[str, str]:
        ...
        # 用户反馈拼到 user message 里，不污染 system prompt
...
请根据用户反馈修改提示词，只修改用户指出的问题，保持其他部分不变。"""
    ...
    def polish_r2v_prompt(self, draft_prompt: str, slots: List[Dict[str, str]], feedback: str = "", custom_system_prompt: str = "") -> Dict[str, str]:
        ...
        # 把参考槽位整理成 character1/2/3 的上下文格式
        slot_context = []
